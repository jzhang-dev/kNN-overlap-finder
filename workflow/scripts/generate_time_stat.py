...
def process_overlap_sizes_file(filename: str):
    ...
    all_mrss,all_clock_time,all_cpu_time,all_dim_time,all_ann_time,samples,regions,platforms,encodes,methods,threads = [],[],[],[],[],[],[],[],[],[],[]
    ...
    with open(nn_time_file) as f:
        ...
        if 'dimension_reduction' in time_dict:
            ...
            all_dim_time.append(0)
        if 'nearest_neighbors' in time_dict:
            all_ann_time.append(time_dict['nearest_neighbors'])
        else:
            all_ann_time.append(0)
    return all_mrss,all_clock_time,all_cpu_time,all_dim_time,all_ann_time,samples,regions,platforms,encodes,methods,threads
...
if not os.path.exists(df_file):
    all_mrss,all_clock_time,all_cpu_time,all_dim_time,all_ann_time,samples,regions,platforms,encodes,methods,threads= process_overlap_sizes_file(filename)
    di = {'thread':threads,'sample':samples,'region':regions,'platform':platforms,'encode':encodes,'method':methods,
        'mrss':all_mrss,'clock_time':all_clock_time,'cpu_time':all_cpu_time,'dimension_reduction_time':all_dim_time,'get_neighbors_time':all_ann_time}
    df = pd.DataFrame(di)
