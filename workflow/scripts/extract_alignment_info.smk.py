...
def get_bam_stat_df(bam_file: str,
    ...
    for i, seg in enumerate(bam.fetch()):
        if (seg.reference_name == chromosome) and (seg.reference_start > start ) and (seg.reference_end < end):
...
def main(snakemake: "SnakemakeContext"):
    ...
    meta_df = top_hits[top_hits.percentage>=0.9]
    meta_df.to_csv(snakemake.output['tsv'], sep='\t', index=False)

    ##stastic if these bad reads have more than one alignment in this region
    all_reads_num = len(top_hits['query_name'].tolist())
    bad_query = top_hits[top_hits.percentage<0.9]['query_name'].tolist()
    ...
    with gzip.open(snakemake.output['fasta'], "w") as fasta_out:
