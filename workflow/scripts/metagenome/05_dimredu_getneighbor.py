...
data = sp.vstack([ref_fm,ref_fm_rp])
data[data > 0] = 1
data = TfidfTransformer(use_idf=True, smooth_idf=True).fit_transform(data)
reducer = random_projection.SparseRandomProjection(n_components=2000) 
_ref = reducer.fit_transform(data)
try:
    _ref = _ref.astype(np.float16)
except Exception as e:
    pickle.dump(_ref,open('/home/miaocj/docker_dir/kNN-overlap-finder/data/feature_matrix/metagenome/GTDB/sample1/GTDB_IDF_2000d.pkl','wb'),protocol=4)
else:
    pickle.dump(_ref,open('/home/miaocj/docker_dir/kNN-overlap-finder/data/feature_matrix/metagenome/GTDB/sample1/GTDB_IDF_2000d.pkl','wb'),protocol=4)
